...
// inherited silently.
func renderConfig(name, mainDir, baseBranch string, prefixes []string, carry []string, agent string) string {
	var b strings.Builder
	...
	fmt.Fprintf(&b, "# and copies the agent's own gitignored settings — permissions, hooks — into\n")
	fmt.Fprintf(&b, "# each new worktree. Remove it for a window with no agent in it.\n")
	if agent == "" {
